...
public class ServiceCRUD {
    ...
    public Optional<Student> findByPIB(String firstName, String lastName, String middleaName) {
        return students.stream().filter(s -> s.getFirstName() == firstName&&s.getLastName() == lastName
        &&s.getMiddleName() == middleaName).findFirst();
    }
    ...
    public void searchingByPIB(String firstName, String lastName, String middleName){
        Optional<Student> optionalStudent = findByPIB(firstName, lastName, middleName);
        if(optionalStudent.isPresent()){
            Student s = optionalStudent.get();
    ...
    public void searchingByGroup(int group){
        List<Student> optionalStudent = findByCourse(group);
        if (optionalStudent.isEmpty()) {
            System.out.println("Студенти не знайдені.");
        } else {
            for (Student s : optionalStudent) {
